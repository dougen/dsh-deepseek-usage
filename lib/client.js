...
// 输出，图标 + 数字，位于卡片右下角）+ 峰谷指示灯（右上角，只有颜色）+ 距下次
// 峰谷切换的倒计时（`2d 15h` / `5h 47m` / `23m`，与指示灯同行、位于其左侧）。
//
// 设计约束（用户只关心"我还能花多少、这会儿什么价"）：
//   · 插件的内部信息——计价来源、同步状态、价格表文件名与路径、模型改名——一律
//     不出现在 UI 上（历史上圆点的 title 里塞过这些，已删除）。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// PEAK_WEEKDAYS_ONLY 保持一致；浏览器侧无法 import 宿主模块，只能镜像）----
		...
		/** 某时刻是否为高峰时段（按北京时间；周末恒为空闲）。与 lib/pricing.js 的 isPeak 等价。 */
		function isPeak(ms) {
		...
		function UsageCell(props) {
			...
			// 快速重试：阶梯下标 + 定时器（宿主正在后台同步时，几秒后自己再看一眼）。
			...
			// 判断依据只看"有没有拿到余额"，不再依赖宿主内部的同步状态字段。
			const load = useCallback(async (force) => {
				try {
					const res = await fetch(USAGE_PATH + "?lang=" + lang + (force ? "&refresh=1" : ""), { cache: "no-store" });
					let body = null;
					try {
						body = await res.json();
					} catch {}
			...
			// 插件的内部信息（计价来源、同步状态、价格表文件名/路径、模型改名）一律不进 UI，
			...
			// 圆点自身没有任何 hover（历史上的 title 里塞了同步时间、价格表路径等内部信息，
			...
			// 注意：这里不显示任何插件内部信息（来源、同步时间、价格表路径），历史上那批
			// 良性提示已随"去技术化"一起删除。
			// 窄卡片（!wide）没有文字空间：仍显示余额与圆点，错误只写进日志。
			if (blanked) {
				return h("div", {
					style: {
						boxSizing: "border-box",
